...
import { markChatRead } from "@/lib/uazapi/client";
import { getChannelToken } from "@/lib/uazapi/channel-token";
import { NextResponse } from "next/server";

/**
 * POST /api/conversations/[id]/chat/read
 * Marcar chat como lido ou não lido no WhatsApp (UAZAPI).
 * Body: { read: boolean }
 */
export async function POST(
  ...
  const result = await markChatRead(resolved.token, number, read);
  if (!result.ok) {
    return NextResponse.json(
      { error: "UAZAPI mark read failed", details: result.error },
      { status: 502 }
    );
  }
  return NextResponse.json({ response: "Chat read status updated successfully" });
}
