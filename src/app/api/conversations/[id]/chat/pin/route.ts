...
import { pinChat } from "@/lib/uazapi/client";
import { getChannelToken } from "@/lib/uazapi/channel-token";
import { NextResponse } from "next/server";

/**
 * POST /api/conversations/[id]/chat/pin
 * Fixar ou desafixar chat no WhatsApp (UAZAPI).
 * Body: { pin: boolean }
 */
export async function POST(
  ...
  const result = await pinChat(resolved.token, number, pin);
  if (!result.ok) {
    return NextResponse.json(
      { error: "UAZAPI pin failed", details: result.error },
      { status: 502 }
    );
  }
  return NextResponse.json({ response: pin ? "Chat pinned" : "Chat unpinned" });
}
